...
namespace MySqlSugar
{
    ...
    public class SqlHelper : IDisposable
    {
        ...
        public bool IsUsePool()
        {
            ...
            try
            {
                string isUsePool = System.Configuration.ConfigurationManager.AppSettings["IsUsePool"]; //这里可以动态根据cookies或session实现多库切换
                if (isUsePool == "1")
                {
                    result = true;
                }
        ...
        public SqlHelper(string connectionString)
        {
            if (IsUsePool())
            {
                _sqlConnection = ConnectionPool.GetPool().GetConnection();
            }
            else
            {
        ...
        public void Dispose()
        {
            if (IsUsePool())
            {
                ConnectionPool.GetPool().CloseConnection(_sqlConnection);
            }
